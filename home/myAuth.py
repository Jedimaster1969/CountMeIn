...
def check_user_linked_to_asset(user_id, asset_id):
    ...
    try:
        user_mapping = Asset_User_Mapping.objects.get(user_ID=user_id, asset_ID=asset_id)
...
def check_user_activated(user_id, asset_id):

    # this function can be used by any view
    # to check if the user is an activated user
    # other pages by editing the URL

    try:
        user_mapping = Asset_User_Mapping.objects.get(user_ID=user_id, asset_ID=asset_id, is_activated=True)
